...
public class Slugma extends AbstractPokemonAlly
{
    public static final String ID = PokemonRegions.makeID(Slugma.class.getSimpleName());
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String NAME = cardStrings.NAME;

    public Slugma(final float x, final float y, AbstractAllyPokemonCard allyCard) {
    ...
    public void takeTurn() {
        ...
        switch (this.nextMove) {
            case MOVE_1: {
                applyToTarget(target, this, new Burn(target, pokeregions.cards.pokemonAllyCards.act2.Slugma.MOVE_1_EFFECT));
                break;
            }
            case MOVE_2: {
                block(adp(), pokeregions.cards.pokemonAllyCards.act2.Slugma.MOVE_2_BLOCK);
                makeInHand(new com.megacrit.cardcrawl.cards.status.Burn(), pokeregions.cards.pokemonAllyCards.act2.Slugma.MOVE_2_STATUS);
                break;
            }
        }
        postTurn();
