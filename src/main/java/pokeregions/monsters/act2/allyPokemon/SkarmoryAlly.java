...
public class SkarmoryAlly extends AbstractPokemonAlly
{
    public static final String ID = PokemonRegions.makeID(Skarmory.class.getSimpleName());
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String NAME = cardStrings.NAME;

    public SkarmoryAlly(final float x, final float y, AbstractAllyPokemonCard allyCard) {
    ...
    public void takeTurn() {
        ...
        switch (this.nextMove) {
            case MOVE_1: {
                applyToTarget(adp(), this, new MetallicizePower(adp(), Skarmory.MOVE_1_EFFECT));
                break;
            }
            case MOVE_2: {
                block(adp(), Skarmory.MOVE_2_EFFECT);
                break;
            }
        }
        postTurn();
