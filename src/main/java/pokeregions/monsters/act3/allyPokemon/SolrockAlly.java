...
public class SolrockAlly extends AbstractPokemonAlly
{
    public static final String ID = PokemonRegions.makeID(Solrock.class.getSimpleName());
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String NAME = cardStrings.NAME;

    public SolrockAlly(final float x, final float y, AbstractAllyPokemonCard allyCard) {
        ...
        switch (this.nextMove) {
            case MOVE_1: {
                for (AbstractMonster mo : Wiz.getEnemies()) {
                    applyToTarget(mo, this, new Burn(mo, Solrock.MOVE_1_EFFECT));
                }
                break;
            }
            case MOVE_2: {
                atb(new AddTemporaryHPAction(adp(), this, Solrock.MOVE_2_EFFECT));
                break;
            }
        }
        postTurn();
