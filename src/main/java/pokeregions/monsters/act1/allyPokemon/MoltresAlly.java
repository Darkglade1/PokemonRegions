...
public class MoltresAlly extends AbstractPokemonAlly
{
    public static final String ID = PokemonRegions.makeID(Moltres.class.getSimpleName());
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String NAME = cardStrings.NAME;

    public MoltresAlly(final float x, final float y, AbstractAllyPokemonCard allyCard) {
        ...
        addMove(MOVE_1, move1Intent, Moltres.MOVE_1_DAMAGE, Moltres.MOVE_1_HITS);
    ...
    public void takeTurn() {
        ...
        switch (this.nextMove) {
            case MOVE_1: {
                ...
                for (int i = 0; i < multiplier; i++) {
                    applyToTarget(target, this, new Burn(target,  Moltres.MOVE_1_BURN));
                }
                break;
            }
            case MOVE_2: {
                atb(new ExhaustDrawPileAction(Moltres.MOVE_2_EXHAUST, true));
                break;
            }
        }
        postTurn();
